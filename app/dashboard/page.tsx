  ...
  const backfillSavings = async (quest: Quest) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        alert(`✅ ${result.newCompletionsAdded} yeni gün eklendi!\n\nBulunan günler: ${result.datesFound?.map((d: any) => `${d.date}: $${d.amount}`).join(', ')}`);
      } else {
                ...
                return (
                  <div key={q.id} className={`flex items-center justify-between p-5 rounded-2xl border ${done ? 'bg-emerald-500/10 border-emerald-500/30' : 'bg-black/25 border-white/5'}`}>
                    ...
                    <div className="flex items-center gap-3">
                      {q.is_savings_quest && (
                        <button 
                          onClick={() => backfillSavings(q)} 
                          disabled={backfilling === q.id}
                          className="flex items-center gap-2 px-3 py-2.5 rounded-xl text-sm font-medium bg-purple-500/20 text-purple-400 hover:bg-purple-500/30"
                          title="Geçmiş transferleri tara"
                        >
                          {backfilling === q.id ? <Loader2 size={14} className="animate-spin" /> : <Calendar size={14} />}
                          {backfilling === q.id ? '...' : 'Geçmişi Tara'}
                        </button>
                      )}
